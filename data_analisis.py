    ...
    if(update.message.chat["type"] == "group"):
        ...
        with open('data.json', 'r+') as file:
            ...
            if(data["Grupos"].get(group_id, False) != False and user_id in data["Grupos"][group_id]["Administradores"]
                ...
                personal.to_csv("CSV/Resultados en la persona " + name +".csv", sep=',')
                ...
                document = open("CSV/Resultados en la persona "+ name +".csv", 'rb')
                )

                os.unlink("CSV/Resultados en la persona " + name + ".csv")
